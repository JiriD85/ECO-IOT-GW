<template>
  <v-container fluid>
    <v-row>
      <v-col cols="12">
        <h1 class="text-h4 mb-4">Network</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          ...
          >
            <v-tab
              ...
              :key="route.name"
              :value="`/network/${route.path}`"
              :to="`/network/${route.path}`"
            >
...
const childRoutes = computed(() => {
  const networkRoute = router.getRoutes().find(r => r.name === 'Network')
  return networkRoute?.children || []
})
