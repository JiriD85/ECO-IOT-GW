<template>
  <article class="panel meter-card" :class="[device.displayLink, { 'temperature-card': device.role === 'temperature' }]">
    ...
    <div v-if="readings.length" class="primary-readings">
      <div v-for="group in groups" :key="group[0].tag" :class="{ 'paired-temperatures': group.length > 1, 'reading-stale': group.every(stale), 'compact-reading': !showTrend(group), 'energy-counter': isEnergy(group[0].tag) }" :data-tone="tone(group[0].tag)">
        <div class="reading-values"><div v-for="r in group" :key="r.tag" :data-tone="tone(r.tag)" :class="{ 'reading-stale': stale(r), 'flow-reading': r.tag === 'Vdot_m3h' }"><span v-if="device.role !== 'temperature'" class="reading-label"><svg v-if="isEnergy(r.tag) || ['T_flow_C', 'T_return_C'].includes(r.tag)" class="reading-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path :d="isEnergy(r.tag) ? 'M3 6h18v12H3z M9 6v12m6-12v12 M6 10v4m6-4v4m6-4v4' : 'M9 14V5a3 3 0 0 1 6 0v9a5 5 0 1 1-6 0Z M12 7v11'"/></svg>{{ group.length > 1 ? (r.tag === 'T_flow_C' ? 'Flow' : 'Return') : label(r.tag) }}</span><div class="reading-number" :title="`${format(r.value)} ${r.unit || ''}`">{{ format(r.value) }} <small>{{ r.unit }}</small></div><small v-if="stale(r)" class="error-text">Last known</small></div></div>
...
const isEnergy = tag => ['E_th_heating_kWh', 'E_th_cooling_kWh'].includes(tag)
