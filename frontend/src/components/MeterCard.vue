<template>
  <article class="panel meter-card" :class="[device.displayLink, { 'temperature-card': device.role === 'temperature' }]">
    ...
    <div v-if="readings.length" class="primary-readings">
      <div v-for="group in groups" :key="group[0].tag" :class="{ 'paired-temperatures': group.length > 1, 'reading-stale': group.every(stale), 'compact-reading': !showTrend(group), 'energy-counter': isCounter(group[0].tag) }" :data-tone="tone(group[0].tag)">
...
const isCounter = tag => ['V_m3', 'E_th_heating_kWh', 'E_th_cooling_kWh'].includes(tag)
